...
interface IHeroesAction {
    type: 'getUserHeroes' | 'setHeroesToBattle' | 'setEnemyHeroes' | 'setUserHeroes';
    payload: IHero[];
}
...
export const heroesReducer = (
    ...
    switch (action.type) {
        ...
        case 'getUserHeroes': {
            ...
            return {...state, userHeroes: userHeroesData ? JSON.parse(userHeroesData) : []}
        }
        case 'setHeroesToBattle': {
            localStorage.setItem('heroesToBattle',
                JSON.stringify([...action.payload
                    .reduce((acc: any, hero: IHero) => [...acc, hero.id], []),])
            );
        ...
        case 'setEnemyHeroes': {
            ...
            if (allEnemyData) {
                const enemyHeroes = filterHeroesOnParse(JSON.parse(allEnemyData)); // если есть уже противники, то парсим и фильтруем
                return {...state, enemyHeroes};
            } else {
                const allEnemy = heroes
                    .filter((item) => action.payload.length === 2
